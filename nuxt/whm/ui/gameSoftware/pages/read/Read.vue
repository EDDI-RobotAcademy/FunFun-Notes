  ...
  <v-container>
    ...
    <v-row v-else>
      <v-col cols="12">
        <!-- 이미지 표시 -->
        <v-img
          :src="getGameSoftwareImageUrl(gameSoftware.image)"
          aspect-ratio="1"
          ...
          <v-card-text>
            <v-container>
              ...
              <v-row>
                <v-col cols="12">
                  <v-text-field
                    v-model="gameSoftware.description"
                    readonly
                    label="상품 설명"
                  />
                </v-col>
...
onMounted(async () => {
  ...
  const software = await gameSoftwareStore.requestGameSoftwareById(
    id as string
  );
  if (software) {
